...
import {
  ...
  ChevronUp, ExternalLink, Copy, Download, FileText,
  CheckCircle, AlertCircle, PlayCircle, XCircle, Flag,
} from 'lucide-react';
import {
...
/* ═══════════════════════════════════════════════════════════════
   SESSION INVESTIGATION WORKSPACE — SOC-Grade Analyst Workbench
   - Workflow status lifecycle
   - Case notes & audit trail
   - Enhanced timeline with narrative stages
   - Action panel with status controls
   ═══════════════════════════════════════════════════════════════ */

// Session Status Lifecycle
const SESSION_STATUSES = [
  { value: 'new', label: 'New', color: 'severity-info', icon: PlayCircle },
  { value: 'under_review', label: 'Under Review', color: 'severity-medium', icon: Eye },
  { value: 'escalated', label: 'Escalated', color: 'severity-high', icon: Flag },
  { value: 'confirmed_scam', label: 'Confirmed Scam', color: 'severity-critical', icon: AlertCircle },
  { value: 'false_positive', label: 'False Positive', color: 'text-muted', icon: XCircle },
  { value: 'closed', label: 'Closed', color: 'severity-low', icon: CheckCircle },
];

export default function Investigation() {
  const [searchParams] = useSearchParams();
  const sessionId = searchParams.get('sid') || 'demo-session';
  ...
  const [sessionStatus, setSessionStatus] = useState('under_review');
  const [caseNotes, setCaseNotes] = useState('Session opened for investigation.');
    ...
    try {
      ...
      if (session && session.session) {
        ...
        const formattedMessages = history.map((msg, i) => ({
          role: msg.role === 'scammer' ? 'scammer' : 'agent',
          text: msg.text,
        ...
        const lastScammerMsg = history.filter(m => m.role === 'scammer').pop();
        if (lastScammerMsg) {
          try {
  ...
  const currentStatusObj = SESSION_STATUSES.find(s => s.value === sessionStatus) || SESSION_STATUSES[1];

  return (
    <div className="space-y-4">
      {/* Session Status Bar */}
      <div className="bg-surface-800 border border-border rounded-lg p-3 sm:p-4">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 lg:gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium uppercase tracking-wider text-text-muted">Session Status</span>
              <currentStatusObj.icon size={14} className={`text-${currentStatusObj.color}`} />
            </div>
            <div className="flex flex-wrap gap-2">
              {SESSION_STATUSES.map(status => (
                <button
                  ...
                  className={clsx(
                    'px-2.5 sm:px-3 py-1.5 rounded-md text-xs font-medium transition-all',
                    sessionStatus === status.value
                      ? `bg-${status.color}/15 text-${status.color} border border-${status.color}/20`
                      : 'bg-surface-700 text-text-muted hover:text-text-secondary border border-transparent'
                  )}
                >
                  {status.label}
                </button>
          ...
          <div className="flex items-center gap-2 text-xs text-text-muted">
            <Clock size={12} />
            <span>Opened: 2h 23m ago</span>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:h-[calc(100vh-14rem)]">
      ...
      <div className="lg:col-span-4 flex flex-col gap-4 overflow-y-auto min-h-[500px] lg:min-h-0">
        ...
        {/* Case Notes & Audit Trail */}
        <Panel title="Case Notes" subtitle="Analyst documentation & audit trail">
          <div className="space-y-3">
            <textarea
              value={caseNotes}
              onChange={e => setCaseNotes(e.target.value)}
              placeholder="Document findings, hypotheses, and decision rationale..."
              className="w-full bg-surface-700 border border-border rounded-md px-3 py-2 text-xs text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent/50 resize-none font-mono"
              rows={8}
            />
            ...
            <div className="border-t border-border pt-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] uppercase tracking-wider text-text-muted">Audit Trail</span>
              </div>
              <div className="space-y-1.5 max-h-32 overflow-y-auto">
                <div className="text-[10px] text-text-muted font-mono">
                  <span className="text-accent">14:32:18</span> → <span className="text-text-secondary">Status changed</span>: New → Under Review <span className="text-text-muted">(Analyst: jsmith)</span>
                </div>
                <div className="text-[10px] text-text-muted font-mono">
                  <span className="text-accent">14:35:42</span> → <span className="text-text-secondary">PII revealed</span>: UPI IDs (3), Phone Numbers (1) <span className="text-text-muted">(Analyst: jsmith)</span>
                </div>
                <div className="text-[10px] text-text-muted font-mono">
                  <span className="text-accent">14:38:11</span> → <span className="text-text-secondary">Case note added</span> <span className="text-text-muted">(Analyst: jsmith)</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Action Panel */}
        <Panel title="Analyst Actions" subtitle="Session control & escalation">
          <div className="space-y-3">
            {/* Primary Actions */}
            <div className="grid grid-cols-2 gap-2">
              <Button 
                variant={sessionStatus === 'escalated' ? 'danger' : 'primary'} 
                size="sm" 
                className="justify-center"
                onClick={() => setSessionStatus('escalated')}
              >
                <Flag size={12} />
                {sessionStatus === 'escalated' ? 'Escalated' : 'Escalate'}
              </Button>
              <Button 
                variant={sessionStatus === 'confirmed_scam' ? 'danger' : 'secondary'} 
                size="sm" 
                className="justify-center"
                onClick={() => setSessionStatus('confirmed_scam')}
              >
                <AlertCircle size={12} />
                Confirm Scam
              </Button>
            ...
            <div className="border-t border-border pt-3 grid grid-cols-2 gap-2">
              ...
              <Button 
                ...
                size="sm" 
                className="justify-center"
                onClick={() => setSessionStatus('false_positive')}
              >
                <XCircle size={12} />
                False Positive
              </Button>
