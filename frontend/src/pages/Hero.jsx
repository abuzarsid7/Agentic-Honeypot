  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 overflow-x-hidden ${
      ...
      <section className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12 lg:pt-16 pb-0 overflow-visible">
        ...
        {/* Live Stats Grid */}
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 max-w-5xl mx-auto animate-fade-in-delay-2 px-4 mb-20">
            {[
              { icon: Activity, value: stats.sessionsAnalyzed || 0, label: 'Sessions Analyzed', color: darkMode ? 'text-accent' : 'text-blue-600' },
              { icon: AlertTriangle, value: stats.scamsDetected || 0, label: 'Scams Detected', color: darkMode ? 'text-severity-critical' : 'text-red-600' },
              { icon: TrendingUp, value: `${stats.avgLatency || 0}ms`, label: 'Avg Response', color: darkMode ? 'text-severity-medium' : 'text-yellow-600' },
              { icon: CheckCircle, value: '99.9%', label: 'Accuracy', color: darkMode ? 'text-severity-low' : 'text-green-600' }
            ].map((stat, i) => (
              <div key={i} className="relative group">
                <div className={`absolute inset-0 rounded-xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity ${
                  darkMode ? 'bg-accent/10' : 'bg-blue-200/50'
                }`}></div>
                <div className={`relative backdrop-blur-sm border rounded-xl p-4 sm:p-6 transition-all ${
                  darkMode
                    ? 'bg-surface-800/40 border-border/50 hover:border-accent/30'
                    : 'bg-white/80 border-slate-200 hover:border-blue-300 shadow-lg'
                }`}>
                  <stat.icon size={24} className={`${stat.color} mb-2 sm:mb-3 mx-auto sm:w-7 sm:h-7`} />
                  <div className={`text-2xl sm:text-3xl lg:text-4xl font-bold font-mono mb-1 ${
                    darkMode ? 'text-text-primary' : 'text-slate-900'
                  }`}>{stat.value}</div>
                  <div className={`text-[10px] sm:text-xs uppercase tracking-wider ${
      ...
      <section id="report" className={`relative max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-20 lg:py-24 border-t ${
        darkMode ? 'border-border/30' : 'border-slate-200'
      }`}>
        <div className="text-center mb-8 sm:mb-12">
          <div className={`inline-block px-3 sm:px-4 py-1 sm:py-1.5 rounded-full border text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-3 sm:mb-4 ${
            darkMode ? 'bg-accent/10 border-accent/20 text-accent' : 'bg-teal-50 border-teal-200 text-teal-700'
          }`}>
            India Compliance
          </div>
          <h2 className={`text-3xl sm:text-4xl lg:text-5xl font-bold mb-3 sm:mb-4 px-4 ${
          ...
          <p className={`text-base sm:text-lg max-w-2xl mx-auto mb-4 px-4 ${
            darkMode ? 'text-text-secondary' : 'text-slate-600'
          }`}>
            Guided complaint filing with correct jurisdiction mapping and audit-ready evidence packaging for Indian regulatory portals
          </p>
          <p className={`text-xs max-w-xl mx-auto px-4 ${
            darkMode ? 'text-text-muted' : 'text-slate-500'
          }`}>
            This platform assists in reporting — final complaint submission occurs on official government portals.
          </p>
        </div>

        {/* Authority Portal Cards */}
          ...
          {[
            {
              ...
              badge: 'Telecom / DoT',
              desc: 'Report phone number misuse, SIM fraud, SMS scams, and spoofed caller IDs.',
        ...
        {/* Evidence & Checklist Summary Row */}
        <div className="grid sm:grid-cols-2 gap-4 sm:gap-6 mb-10">
          {/* Evidence Packet */}
          <div className={`backdrop-blur-sm border rounded-xl p-5 sm:p-7 ${
            darkMode ? 'bg-surface-800/40 border-border/50' : 'bg-white/80 border-slate-200 shadow-lg'
          }`}>
            <div className="flex items-center gap-2 mb-4">
              <FileText size={18} className={darkMode ? 'text-accent' : 'text-teal-600'} />
              <h3 className={`text-sm sm:text-base font-bold ${
                darkMode ? 'text-text-primary' : 'text-slate-900'
              }`}>Evidence Packaging</h3>
            </div>
            <ul className="space-y-2">
              {[
                'Auto-generated scam summary (plain language)',
                'Risk verdict with confidence score',
                'Chronological conversation timeline',
                'Extracted artifacts (UPI IDs, phones, URLs)',
                'PII masking controls',
                'One-click PDF / ZIP export'
              ].map((item, i) => (
                <li key={i} className={`flex items-start gap-2 text-xs sm:text-sm ${
                  darkMode ? 'text-text-secondary' : 'text-slate-600'
                }`}>
                  <CheckCircle size={14} className={`shrink-0 mt-0.5 ${
                    darkMode ? 'text-severity-low' : 'text-green-500'
                  }`} />
                  {item}
                </li>
              ))}
          ...
          <div className={`backdrop-blur-sm border rounded-xl p-5 sm:p-7 ${
            ...
            <div className="flex items-center gap-2 mb-4">
              <CheckCircle size={18} className={darkMode ? 'text-severity-medium' : 'text-yellow-600'} />
              <h3 className={`text-sm sm:text-base font-bold ${
                darkMode ? 'text-text-primary' : 'text-slate-900'
              }`}>Before You File</h3>
            </div>
            <ul className="space-y-2">
              {[
                'Scam confirmed (High / Critical score)',
                'Financial loss assessment documented',
                'Reporting within 24 hours (recommended)',
                'Bank contacted (for UPI fraud)',
                'Evidence packet downloaded',
              ].map((item, i) => (
                <li key={i} className={`flex items-start gap-2 text-xs sm:text-sm ${
                  darkMode ? 'text-text-secondary' : 'text-slate-600'
                }`}>
                  <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 mt-0.5 ${
                    darkMode ? 'border-border bg-surface-700' : 'border-slate-300 bg-slate-50'
                  }`}>
                    <div className={`w-1.5 h-1.5 rounded-sm ${
                      darkMode ? 'bg-text-muted' : 'bg-slate-400'
                    }`} />
                  </div>
                  {item}
                </li>
        ...
        <div className="text-center">
          <button
            ...
            className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 sm:gap-3 px-6 sm:px-8 py-3 sm:py-4 rounded-2xl font-semibold transition-all hover:scale-105 text-base shadow-xl ${
              darkMode
                ? 'bg-gradient-to-r from-accent to-accent-dim text-surface-900 hover:shadow-accent/20'
                : 'bg-gradient-to-r from-teal-600 to-teal-700 text-white hover:shadow-teal-500/30'
            }`}
          >
            <Landmark size={18} className="sm:w-5 sm:h-5" />
            Open Complaint Filing Hub
            <ChevronRight size={18} className="sm:w-5 sm:h-5" />
          </button>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
            ...
            <div className={`flex items-center gap-4 sm:gap-8 text-xs sm:text-sm ${
              ...
              <a href="#" className={darkMode ? 'hover:text-text-primary' : 'hover:text-slate-900'}>Documentation</a>
              <a href="#" className={darkMode ? 'hover:text-text-primary' : 'hover:text-slate-900'}>API Reference</a>
              <a href="#" className={darkMode ? 'hover:text-text-primary' : 'hover:text-slate-900'}>Contact</a>
            </div>
          </div>
